import pytest
from src.ratestack.domain.exceptions import InvalidRegionException
from src.ratestack.domain.models import Regions, Ports, Prices
...
def create_price(origin_port="orcod", dest_port="dscod", date="2011-01-02", price=Decimal("50.0")):
    return Prices(
...
def test_parent_slug_is_empty():
    ...
    assert len(region.parent_slug) == 0
...
def test_add_parent_sucessfully():
    ...
    assert parent_region in child_region.parent_slug
    assert len(parent_region.parent_slug) == 0
...
def test_more_than_5_origin_port_is_invalid():
    prices = create_price(origin_port="ABCDEF", dest_port="XYZ",
                          date=date.today(), price=Decimal("100.00"))
    assert not prices.is_valid()


def test_more_than_5_destination_port_is_invalid():
    prices = create_price(origin_port="ABC", dest_port="XYDZZY",
                          date=date.today(), price=Decimal("100.00"))
    assert not prices.is_valid()


def test_price_negative_is_invalid():
    prices = create_price(origin_port="ABC", dest_port="XYZ",
                          date=date.today(), price=Decimal("-100.00"))
    assert not prices.is_valid()


def test_price_string_is_invalid():
    price = create_price(origin_port="ABC", dest_port="XYZ",

...
def create_price(
    orig_code="orcod", dest_code="dscod", date="2011-01-02", price=Decimal("50.0")
):
    return Prices(orig_code=orig_code, dest_code=dest_code, price=price, date=date)
...
def test_add_invalid_parent():
    ...
    invalid_region = create_region(slug="invalid slug", name="special slug name")
...
def test_add_parent_sucessfully():
    child_region = create_region()
    parent_region = create_region(slug="slug", name="special slug name")
...
def test_create_port():
    port_fields = {"code": "asef", "name": "code name", "parent_slug": "slug_name"}
...
def test_more_than_5_orig_code_is_invalid():
    prices = create_price(
        orig_code="ABCDEF", dest_code="XYZ", date=date.today(), price=Decimal("100.00")
    )
...
def test_more_than_5_destination_port_is_invalid():
    prices = create_price(
        orig_code="ABC", dest_code="XYDZZY", date=date.today(), price=Decimal("100.00")
    )
    assert not prices.is_valid()
...
def test_price_negative_is_invalid():
    prices = create_price(
        orig_code="ABC", dest_code="XYZ", date=date.today(), price=Decimal("-100.00")
    )
    assert not prices.is_valid()
...
def test_price_string_is_invalid():
    price = create_price(
        orig_code="ABC", dest_code="XYZ", date=date.today(), price="not a decimal"
    )
...
def test_not_valid_date_prices_is_invalid():
    prices = create_price(
        orig_code="ABC", dest_code="XYZ", date="2022-15-01", price=Decimal("100.00")
    )
    assert not prices.is_valid()
...
def test_price_is_valid():
    prices = create_price(
        orig_code="ABC", dest_code="XYZ", date="2022-12-23", price=Decimal("100.00")
    )
